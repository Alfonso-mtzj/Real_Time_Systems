 ...
 *
 * ============================================================
 * Theme: ThemePark    <-- e.g., Avionics, Medical, Space, Industrial, Security
 * ============================================================
 */
...
static void blink_task(void *arg)
{
    ...
    for (;;) {
        ...
        ESP_LOGI(TAG, "[ThemePark] dispatch beacon = %s (cycle #%lu)",
                 led_on ? "Ready" : "Standby", (unsigned long)toggle_count);
...
static esp_err_t handle_root(httpd_req_t *req)
{
    ...
    static const char html[] =
        ...
        "</style></head>"
        "<body>"
        "<h1>ThemePark monitor</h1>"
        "<p>Ride dispatch status:</p>"
        ...
        "    document.getElementById('label').textContent = s.on ? 'Ready' : 'Standby';"
...
static httpd_handle_t start_webserver(void)
{
    ...
    if (httpd_start(&server, &cfg) == ESP_OK) {
      ESP_LOGI(TAG, "[ThemePark] HTTP server successfully running");
        httpd_uri_t root = {
            .uri = "/",
            .method = HTTP_GET,
            .handler = handle_root,
        ...
        ESP_LOGI(TAG, "[ThemePark] operator console started on port %d", HTTP_PORT);
    } else {
        ESP_LOGE(TAG, "[ThemePark] operator console failed to start");
    }
    return server;
}
...
static void wifi_event_handler(void *arg, esp_event_base_t base, int32_t id, void *data)
{
    if (base == WIFI_EVENT && id == WIFI_EVENT_STA_START) {
        ...
        ESP_LOGW(TAG,  "[ThemePark] Wi-Fi disconnected; reconnecting...");
        esp_wifi_connect();
    } else if (base == IP_EVENT && id == IP_EVENT_STA_GOT_IP) {
        ip_event_got_ip_t *event = (ip_event_got_ip_t *)data;
        ESP_LOGI(TAG, "[ThemePark] operator console IP: " IPSTR, IP2STR(&event->ip_info.ip));
        start_webserver();
    }
}
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "==== App 1 [ThemePark] starting ====");
    ...
    xTaskCreatePinnedToCore(
        blink_task,        /* function */
        "dispatch_beacon",           /* name (max 16 chars) */
